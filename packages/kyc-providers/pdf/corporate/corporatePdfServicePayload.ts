...
export const RISK_PROFILES = ["Low Risk", "Moderate", "High Risk"] as const;
...
function pickPrimaryBankAccount(accounts: BankAccount[]): BankAccount | undefined {
  return accounts.find((a) => a.isPrimary === "Yes") ?? accounts[0];
}

function pickPrimaryDematAccount(accounts: DematAccount[]): DematAccount | undefined {
  return accounts.find((a) => a.isPrimary === "Yes") ?? accounts[0];
}
...
function formatAddressForUbo(addr?: Address): string {
  if (!addr) return "";
...
function mapGenderToUboCode(gender?: Gender): string {
  if (gender === "Male") return "M";
  if (gender === "Female") return "F";
  if (gender === "Transgender") return "T";
  return "";
}

function namesMatch(a?: string, b?: string): boolean {
  const left = trimOrEmpty(a).toLowerCase();
  const right = trimOrEmpty(b).toLowerCase();
  return Boolean(left && right && left === right);
}

function hydrateUboFromRelatedPerson(
  ubo: UBO,
  relatedPerson: RelatedPerson | undefined,
  isPrimaryRow: boolean,
): UBO {
  if (!relatedPerson) return ubo;

  const useRelated =
    isPrimaryRow || namesMatch(ubo.name, relatedPerson.name);
  if (!useRelated) return ubo;

  const nationalityFromRelated = relatedPerson.nationality?.includes("Indian")
    ? "Indian"
    : "";

  return {
    ...ubo,
    name: trimOrEmpty(ubo.name) || trimOrEmpty(relatedPerson.name),
    fatherName:
      trimOrEmpty(ubo.fatherName) ||
      trimOrEmpty(relatedPerson.fatherOrSpouseName),
    gender: trimOrEmpty(ubo.gender) || mapGenderToUboCode(relatedPerson.gender),
    address:
      trimOrEmpty(ubo.address) ||
      formatAddressForUbo(relatedPerson.correspondenceAddress),
    dateOfBirth:
      trimOrEmpty(ubo.dateOfBirth) || trimOrEmpty(relatedPerson.dateOfBirth),
    countryOfBirth: trimOrEmpty(ubo.countryOfBirth) || "India",
    nationality: trimOrEmpty(ubo.nationality) || nationalityFromRelated || "Indian",
    isUSPerson: ubo.isUSPerson ?? "N",
    countryOfTaxResidence:
      trimOrEmpty(ubo.countryOfTaxResidence) || "India",
    tin: trimOrEmpty(ubo.tin) || trimOrEmpty(relatedPerson.pan),
    idProofSubmitted:
      trimOrEmpty(ubo.idProofSubmitted) ||
      (relatedPerson.proofOfIdentity?.aadhaarChecked ? "PAN + Aadhaar" : "PAN"),
    relationshipWithEntity:
      trimOrEmpty(ubo.relationshipWithEntity) ||
      relatedPerson.relatedPersonTypes?.join(" / ") ||
      "",
  };
}

/**
 * Ensures Page 14 (Annexure 1.2) UBO rows are filled. Page 14 DOB only reads
 * `annexure12.ubos[n].dateOfBirth` — not `relatedPerson.dateOfBirth` (Page 6).
 * Copies from related person into `ubos[0]` (and name-matched rows) when blank.
 */
export function hydrateAnnexure12ForPdfPayload(
  payload: CorporateKycData,
): CorporateKycData {
  const annexure12 = payload.annexure12 ?? {};
  const relatedPerson = payload.relatedPerson;
  let ubos = [...(annexure12.ubos ?? [])];

  if (
    ubos.length === 0 &&
    annexure12.declareWithUBOs &&
    trimOrEmpty(relatedPerson?.name)
  ) {
    ubos = [{}];
  }

  const hydratedUbos = ubos.slice(0, 2).map((ubo, idx) =>
    hydrateUboFromRelatedPerson(ubo, relatedPerson, idx === 0),
  );

  const hasNamedUbos = hydratedUbos.some((u) => trimOrEmpty(u.name));
  const declareWithUBOs =
    annexure12.declareWithUBOs ?? hasNamedUbos;
  const declareNoUBOs = hasNamedUbos
    ? false
    : (annexure12.declareNoUBOs ?? false);

  return {
    ...payload,
    annexure12: {
      ...annexure12,
      declareWithUBOs: hasNamedUbos ? true : declareWithUBOs,
      declareNoUBOs,
      ubos: hydratedUbos,
      name: trimOrEmpty(annexure12.name) || trimOrEmpty(relatedPerson?.name),
      designation:
        trimOrEmpty(annexure12.designation) ||
        trimOrEmpty(relatedPerson?.relatedPersonTypes?.[0]) ||
        "Director",
      dinPan:
        trimOrEmpty(annexure12.dinPan) ||
        [trimOrEmpty(relatedPerson?.din), trimOrEmpty(relatedPerson?.pan)]
          .filter(Boolean)
          .join(" / "),
    },
  };
...
export function hydrateCorporatePdfPayload(
  payload: CorporateKycData,
  options?: HydrateNclAnnexureOptions,
): CorporateKycData {
  return hydrateAnnexure12ForPdfPayload(
    hydrateNclAnnexureForPdfPayload(payload, options),
  );
}

export type HydrateNclAnnexureOptions = {
  /** Customer `userName` — NCL participant code for Page 15 row 2. */
  participantCodeFallback?: string;
};

/**
 * Ensures Page 15 (NCL Annexure) has participant + demat rows filled. Copies
 * from customer userName, `dematAnnexure`, or `additionalInfo.settlementAgency`
 * when NCL values are blank so the external pdf-service renders rows 2, 13, 14, and 15.
 *
 * Page 15 row 2 only reads `nclAnnexure.values.participantCode` — not Page 8 `nclCode`.
 */
export function hydrateNclAnnexureForPdfPayload(
  payload: CorporateKycData,
  options?: HydrateNclAnnexureOptions,
): CorporateKycData {
  const dematAccounts = payload.dematAnnexure?.accounts ?? [];
  const bankAccounts = payload.bankAnnexure?.accounts ?? [];
  const primaryDemat = pickPrimaryDematAccount(dematAccounts);
  const primaryBank = pickPrimaryBankAccount(bankAccounts);

  const ncl = payload.nclAnnexure ?? {};
  const values = ncl.values ?? {};
  const agency = payload.additionalInfo?.settlementAgency ?? {};

  const participantCode =
    trimOrEmpty(values.participantCode) ||
    trimOrEmpty(options?.participantCodeFallback) ||
    trimOrEmpty(agency.nclCode);

  const dpName =
    trimOrEmpty(values.dpName) ||
    trimOrEmpty(primaryDemat?.dpName) ||
    "";
  const dpId =
    trimOrEmpty(values.dpId) || trimOrEmpty(primaryDemat?.dpId) || "";
  const clientId =
    trimOrEmpty(values.clientId) ||
    trimOrEmpty(primaryDemat?.beneficiaryId) ||
    "";
  ...
  return {
    ...
    additionalInfo: {
      ...
      applicantName:
        payload.additionalInfo?.applicantName ?? payload.entity?.name ?? "",
      applicantPan: payload.additionalInfo?.applicantPan ?? payload.entity?.pan ?? "",
      settlementAgency: {
        ...agency,
        nclChecked: agency.nclChecked ?? Boolean(participantCode),
        nclCode: trimOrEmpty(agency.nclCode) || participantCode,
      },
    },
    nclAnnexure: {
      ...ncl,
      date: ncl.date ?? payload.additionalInfo?.date,
      values: {
        ...values,
        participantName:
          trimOrEmpty(values.participantName) ||
          trimOrEmpty(payload.entity?.name),
        participantCode,
        dpName,
        dpId,
        clientId,
        depository:
          values.depository ?? primaryDemat?.depository ?? "NSDL",
        bankName:
          trimOrEmpty(values.bankName) ||
          trimOrEmpty(primaryBank?.bankName),
        bankBranch:
          trimOrEmpty(values.bankBranch) ||
          trimOrEmpty(primaryBank?.branch),
        bankIfsc:
          trimOrEmpty(values.bankIfsc) || trimOrEmpty(primaryBank?.ifsc),
        bankAccountNumber:
          trimOrEmpty(values.bankAccountNumber) ||
          trimOrEmpty(primaryBank?.accountNumber),
      },
      authorisedSignatoryName:
        ncl.authorisedSignatoryName ??
        payload.relatedPerson?.name ??
        "",
      authorisedSignatoryDesignation:
        ncl.authorisedSignatoryDesignation ??
        payload.relatedPerson?.relatedPersonTypes?.[0] ??
        "Director",
    },
  };
}
...
export function mapCorporateKycToPdfPayload(
  ...
  const dematAccounts: DematAccount[] = (data.dematAccounts ?? []).map((acc) => ({
    ...
    dpName:
      trimOrEmpty(acc.dpName) || trimOrEmpty(acc.accountHolderName),
    dpId: trimOrEmpty(acc.dpId),
    beneficiaryId: trimOrEmpty(acc.clientId),
  }));

  const participantCode = trimOrEmpty(customer?.userName);
  ...
  const basePayload: CorporateKycData = {
    applicationType: "New",
    ...
    additionalInfo: {
      ...
      applicantPan: pan,
      settlementAgency: {
        nclChecked: Boolean(participantCode),
        nclCode: participantCode,
        icclChecked: false,
        icclCode: "",
        sglChecked: false,
        sglCode: "",
  ...
  return hydrateCorporatePdfPayload(basePayload, {
    participantCodeFallback: customer?.userName,
  });
}

export type PrepareCorporatePdfServicePayloadArgs = {
  kyc: CorporateKycResponse;
  customer?: Pick<
    CustomerProfile,
    "userName" | "emailAddress" | "phoneNo"
  > | null;
  /** CRM-saved PDF editor JSON (`lastPdfPayload`). When set, used as the base. */
  savedPayload?: Record<string, unknown> | null;
  riskAnswers: Array<{ index: number; ans: string }>;
};

/**
 * Builds the JSON body for `POST /api/corporate/pdf` (pdf-service).
 * Uses saved CRM payload when available; otherwise maps from corporate KYC.
 * Always overlays the customer's risk-profile selections at sign time.
 */
export function prepareCorporatePdfServicePayloadForEsign(
  args: PrepareCorporatePdfServicePayloadArgs,
): Record<string, unknown> {
  const { kyc, customer, savedPayload, riskAnswers } = args;
  ...
  const base = hasSaved
    ? hydrateCorporatePdfPayload(savedPayload as CorporateKycData, {
        participantCodeFallback: customer?.userName,
      })
    : mapCorporateKycToPdfPayload(kyc, customer ?? null);

  const withRisk = applyCorporateRiskProfileToSavedPdfPayload(
    base as Record<string, unknown>,
    riskAnswers,
  );

  return applyParticipantCodeToSavedPdfPayload(
    withRisk,
    customer?.userName?.trim() || undefined,
  );
}
