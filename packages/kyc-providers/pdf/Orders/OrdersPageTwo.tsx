...
Client ID: ${demat?.clientId}`}</Text>
      ...
      <Text style={tw(`text-[8px] mt-3`)}>
        This {releasedOrder ? "" : "Draft "}Order Receipt is a system generated document and does not require any signatures.
      </Text>

      <Text style={tw(`text-[8px] mt-3 font-semibold`)}>Terms & Conditions</Text>
      <Text style={tw(`text-[8px] mt-1 mb-1 leading-5`)}>
        These terms and conditions (“Terms”) form an essential part of the Order Receipt issued by BondNest Capital India Securities Private Limited (“MeraDhan”) to the Buyer for the proposed transaction(s) listed above:
      </Text>
      <View style={tw(`mt-1`)}>
        ...
        <View style={tw(`ml-6 mb-1 flex flex-col`)}>
          <Text style={tw(`text-[8px] leading-5`)}>a. BSE: Penal action for failure to honour RFQ transactions.</Text>
          <Text style={tw(`text-[8px] leading-5`)}>b. NSE: Actions for failure to settle a deal on the RFQ platform – Individual Investors.</Text>
        </View>
        ...
        <TextList className="text-[8px]" count="8.">
          MeraDhan is not responsible for any errors or missing information. For any queries or discrepancies, please write to: support@meradhan.com
        </TextList>
      ...
      <Text style={tw(`text-[8px] mt-3 font-semibold`)}>Disclaimer</Text>
      ...
      <View style={tw(`mt-1 flex flex-col`)}>
        <Text style={tw(`text-[8px] leading-5`)}>a) The Order Date and Time reflects when the RFQ was initiated by the Online Bond Platform on behalf of its client (whether buy or sell)</Text>
        <Text style={tw(`text-[8px] leading-5`)}>b) The Clearing Corporation may credit funds/securities to the settlement Bank/Demat account given above or to the default Bank/Demat account registered with the Clearing Corporation.</Text>
      </View>
      ...
      <View style={tw(`mt-1 flex flex-col`)}>
        ...
        <Text style={tw(`text-[8px] leading-5 ml-2`)}>b) I have reviewed the details in the Order Receipt and wish to proceed with the payment.</Text>
      </View>
      <Text style={tw(`text-[8px] mt-6 text-center`)}>The End</Text>
    </View>
  );
}
