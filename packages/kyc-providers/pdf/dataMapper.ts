...
export type Page1Props = {
    ...
    residentialStatus?: string;
    occupationType?: string;
    verifyWith?: "AADHAAR" | "DL" | "VID" | "PASSPORT" | "NREGA" | "NPR" | "OTHERS";
    profilePic?: string;
    signature?: string;
};
...
export type Page2Props = {
    ...
    proofWith?: "AADHAAR" | "DL" | "VID" | "PASSPORT" | "NREGA" | "NPR" | "OTHERS";
};
...
const getFullName = (data: Root): string => {
    return `${data.step_1?.pan?.firstName || ""} ${data.step_1?.pan?.middleName || ""} ${data.step_1?.pan?.lastName || ""
        }`.trim();
};
...
const getAddress = (data: Root) => {
    ...
    return {
        ...
        combined: `${address?.district_or_city || ""} ${address?.state || ""} ${address?.pincode || ""}`.trim(),
    };
};
...
export const mapDataForPage1 = (data: Root): Page1Props => ({
    ...
    maritalStatus: data.step_2?.maritalStatus as "SINGLE" | "MARRIED" | "OTHERS" || "SINGLE",
    nationality: "INDIAN",
...
export const mapDataForPage6 = async (data: Root): Promise<Page6Props> => ({
    ...
    eAaDhar: data.step_1?.pan?.response?.details?.aadhaar?.file_url ? await pdfUrlToBase64(data.step_1?.pan?.response?.details?.aadhaar?.file_url || "") : "",
});

export const mapDataForPage7 = async (data: Root): Promise<Page7Props> => ({
    ePan: data.step_1?.pan?.response?.details?.pan?.file_url ? await pdfUrlToBase64(data.step_1?.pan?.response?.details?.pan?.file_url || "") : "",
});
...
export const mapDataForPage11 = (data: Root): Page11Props => {
    ...
    const primaryBankAccount = data.step_3?.find((b) => b.isDefault) || data.step_3?.[0];
