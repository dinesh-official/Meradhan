...
type Props = {
  index: number;
  account: BankAccount;
  error?: Partial<Record<keyof BankAccount, string[]>>;
  onChange: <K extends keyof BankAccount>(key: K, value: BankAccount[K]) => void;
  onSetDefault: () => void;
  onRemove: () => void;
};
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="font-bold">Bank Account #{index + 1}</h2>
        {index > 0 && (
          <button
            type="button"
            onClick={onRemove}
