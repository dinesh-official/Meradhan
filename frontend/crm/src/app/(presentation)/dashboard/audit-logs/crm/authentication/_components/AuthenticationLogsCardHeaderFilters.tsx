...
interface AuthenticationLogsCardHeaderFiltersProps {
  filters: GroupQuery;
...
function AuthenticationLogsCardHeaderFilters({
  filters,
  onFilterChange,
  isLoading = false,
  description,
  title,
}: AuthenticationLogsCardHeaderFiltersProps) {
  const [searchInput, setSearchInput] = useState(filters.search || "");
  const [selectedUser, setSelectedUser] = useState<CrmUsersProfile | null>(
    null
  );
  ...
  const handleUserSelect = (user: CrmUsersProfile | null) => {
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <CardDescription className="mt-1">
            {description ||
              "Complete log of all user authentication actions and system events within the CRM"}
          </CardDescription>
        </div>
      </div>
...
export default AuthenticationLogsCardHeaderFilters;
