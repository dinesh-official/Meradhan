 ...
 */
function removeHonorifics(normalizedName: string, customHonorifics?: string[]): string {
 ...
 */
function compareDates(dob1: Date | string | null | undefined, dob2: Date | string | null | undefined): boolean {
    if (!dob1 || !dob2) return false;

    const date1 = dob1 instanceof Date ? dob1 : new Date(dob1);
    const date2 = dob2 instanceof Date ? dob2 : new Date(dob2);
...
export function compareNames(
    ...
    const lastNameExact =
        getLastName(n1) !== null &&
        getLastName(n1) === getLastName(n2);

    const initialStyleDetected =
        isInitialStyleName(n1) || isInitialStyleName(n2);
...
export function getStatusInfo(
    ...
    score: number
): { status: MatchResult["decision"]; label: string; nextStep: string; score: number } {
    switch (decision) {
        case "MATCH_FULL":
            return {
                status: "MATCH_FULL",
