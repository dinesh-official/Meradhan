...
query DynamicPages($pagination: PaginationArg, $filters: DynamicPagesListFiltersInput) {
...
export const getDynamicPageMetaDataGql = async (slug: string): Promise<Metadata> => {
    const { data } = await gqlClient.query<DynamicPagesGqlResponse>({
