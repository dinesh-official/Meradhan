...
export type FrequencyType = "monthly" | "quarterly" | "semi-annual" | "annual" | "maturity";
...
export const prepareXirrValues = (data: Cashflow[]): CashFlow[] => {
    ...
    const filterData = filtered.map((e, i) => {
        return {
            ...
            amount: (i == filtered.length - 1) ? +(e.amount + e.interest).toFixed(2) : e.amount
        }
    })
...
export function getMinMax(arr: number[]): { min: number | null; max: number | null } {
    if (!Array.isArray(arr) || arr.length === 0) {
...
const getValidPaymentDate = (year: number, month: number, targetDay: number): Date => {
...
export const getBondCashflowJson = ({
    ...
    while (true) {
        ...
        if (++safetyCounter > 1000) {
            console.error("Loop exceeded safe limit. Check bond frequency/date logic.");
            break;
        }
    }
