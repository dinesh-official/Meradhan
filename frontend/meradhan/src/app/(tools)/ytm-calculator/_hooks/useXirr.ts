...
import { formatToMMDDYYYY, FrequencyType, getBondCashflowJson } from "../_helpers/xirr";
...
export const useXirr = () => {
    ...
    const validationErrors = useMemo(() => {
        ...
        if (!isValidNumber(faceValue)) errors.push("Face value must be a positive number");
        if (!isValidNumber(cleanPrice)) errors.push("Clean price must be a positive number");
        if (!isValidNumber(couponRate)) errors.push("Coupon rate must be a positive number");
        if (!isValidDate(maturityDate)) errors.push("Invalid maturity date");
        if (!isValidDate(lastCouponDate)) errors.push("Invalid last coupon date");
        if (!isValidDate(settlementDate)) errors.push("Invalid settlement date");
        ...
        if (isValidDate(lastCouponDate) && isValidDate(settlementDate)) {
            if (new Date(lastCouponDate) > new Date(settlementDate)) {
                errors.push("Last coupon date must be before or equal to settlement date");
            }
        }
        
        return errors;
    }, [faceValue, cleanPrice, couponRate, maturityDate, lastCouponDate, settlementDate]);
