...
import type { IFileUploadProvider } from './fileUpload.interface';
...
export class FileUploadProvider implements IFileUploadProvider {
    ...
    constructor(private uploadFolder: string, allowedFileTypes?: string[]) {
        ...
        const storage = multer.diskStorage({
            ...
            destination: (req, file, cb) => {
                const subfolder = new Date().toISOString().split('T')[0] + "/" + (req.session?.id || 'files');
    ...
    async uploadFile(file: Express.Multer.File): Promise<string> {
        ...
        return `${config.hostUrl}${file.destination.split(this.uploadFolder)[1]}/${file.filename}`;
    }
