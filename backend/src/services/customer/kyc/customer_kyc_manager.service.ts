            ...
            if (step3.length > 0) {
                await tx.customersBankAccountModel.createMany({
                    data: step3.map(bank => ({
                        ...
                        verifyDate: bank.isVerified ? bank.confirmBankTimestamp : undefined,
    ...
    async getUserKycFlowDataWithFormattedFullProfile(customerId: number): Promise<any> {
        ...
        const data: Awaited<ReturnType<CustomerProfileService["getFullCustomerProfile"]>> = {
            ...
            personalInformation: step2 || user?.personalInformation ? {
                ...
                confirmTimeStamp: user?.personalInformation?.confirmTimeStamp || step2?.confirmPersonalInfoTimestamp,
