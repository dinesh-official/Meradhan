...
export class ParticipantManager {
    ...
    public async registerParticipant(userId: number) {
        ...
        const address = user.currentAddress!.cityOrDistrict + "," + user.currentAddress!.postOffice + "," + user.currentAddress!.state + "," + user.currentAddress!.pinCode;
        console.log(user.dematAccounts.map((e) => {
            return {
                benId: e.clientId,
        ...
        console.log( user.bankAccounts.map((e) => {
                return {
    ...
    public async getAllParticipants() {
        return await this.cbrics.getAllUnregisteredParticipants({ workflowStatus: 1 });
    }
