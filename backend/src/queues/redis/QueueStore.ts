...
import type { IQueueService } from '../services/interface';
...
export class QueueStore implements IQueueService {
  ...
  public async setKey<T>(key: string, value: T, ttlSeconds?: number): Promise<void> {
  ...
  public async getKey<T>(key: string): Promise<T | null> {
    ...
    try {
      ...
      logger.logInfo(`⚠️ Could not parse key "${key}" as JSON. Returning raw string.`);
