...
export class NSEIsinService {
    ...
    async searchIsin(
        ...
        if (payload.description) {
            filters.description = { contains: payload.description, mode: "insensitive" };
        }
        ...
        const limit = payload.limit && Number(payload.limit) > 0 ? Number(payload.limit) : 10;
        const skip = (page - 1) * Number(limit);

        // 🧮 Count total matching records
        const total = await db.dataBase.nseIsinSecurityReceipt.count({ where: filters });
