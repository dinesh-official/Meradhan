...
import { AuditLogsRepo } from "./audit_logs.repo";
...
export class AuditLogsService {
    ...
    private auth_list = ["login", "logout", "auto_logout", "otp_request", "otp_verify", "session"]

    constructor() {
        this.auditLogsRepo = new AuditLogsRepo();
    }
    ...
    async createTracking(payload: TrackingPayload) {
        ...
        const auditLogData: DataBaseSchema.CrmAuditLogsCreateInput = {
            ...
            url: payload.type === "page_duration"
    ...
    async revalidateTracking(payload: RevalidatePayload) {
        ...
        return await this.auditLogsRepo.updateAuditLogsByTrackId(
            payload.trackId,
            {
    ...
    async getAuditLogsList(query: AuditLogsListQuery) {
        ...
        const where: DataBaseSchema.WebAuditLogsWhereInput = {};
        ...
        if (query.search) {
            ...
            where.OR = [
                ...
                ...(Number.isInteger(searchNum) && searchNum > 0 ? [{ userId: searchNum }] : []),
            ];
        }
    ...
    async getGroupedAuditLogs(query: GroupQuery) {
        ...
        const baseWhere: DataBaseSchema.WebAuditLogsWhereInput = {
            trackId: { not: null },
            type: {
        ...
        return {
            meta: {
                ...
                totalPages: Math.ceil((query.search ? filteredSessions.length : totalGroups.length) / limit),
            },
