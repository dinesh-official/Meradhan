...
export class AuditLogsRepo {
    ...
    async updateAuditLogsByTrackId(trackId: string, data: DataBaseSchema.WebAuditLogsUpdateInput) {
    ...
    async findCRMUser(userId: number) {
        const response = await db.dataBase.customerProfileDataModel.findUnique({
            where: { id: userId },
            select: { id: true, firstName: true, lastName: true, emailAddress: true, middleName: true }
        });
        return {
            id: response?.id || 0,
            name: response ? `${response.firstName} ${response.middleName} ${response.lastName}` : null,
