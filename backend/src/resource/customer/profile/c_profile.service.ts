...
export class CustomerProfileService {
    ...
    async updateMobileRequest({ customerId, newMobile, newWhatsAppNo }: { newMobile: string, newWhatsAppNo?: string, customerId: number }): Promise<boolean> {
        ...
        if (existingOtp) {
            throw new AppError("Mobile number is already associated with another account.");
        }
    ...
    async sendMobileOtpVerification({ newMobile }: { newMobile: string }): Promise<string> {
    ...
    async verifyAndUpdateMobile({ customerId, newMobile, otpToken, otpCode }: { customerId: number, newMobile: string, otpToken: string, otpCode: string }): Promise<boolean> {
        ...
        if (isValid) {
            ...
            console.log(`Mobile number for customer ${customerId} updated to ${newMobile}`);
    ...
    async toggleWhatsAppNotifications(customerId: number, enable: boolean): Promise<boolean> {
        await db.dataBase.customerProfileDataModel.update({
            data: {
                utility: {
                    update: {
