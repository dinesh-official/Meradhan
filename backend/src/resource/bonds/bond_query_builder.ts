...
export class BondQueryBuilder {
     ...
     */
    static generateFilterQuery(filters: z.infer<typeof appSchema.bonds.bondsFilterSchema>): DataBaseSchema.BondsWhereInput {
     ...
     */
    private static addSearchFilter(conditions: DataBaseSchema.BondsWhereInput[], search?: string): void {
        if (!search) return;
    ...
    private static addArrayFilter(
        ...
        conditions.push({ [field]: { in: values } } as DataBaseSchema.BondsWhereInput);
    }

    /**
     * Add maturity date range filter
     */
    private static addMaturityFilter(conditions: DataBaseSchema.BondsWhereInput[], maturity?: string[]): void {
     ...
     */
    private static addCouponFilter(conditions: DataBaseSchema.BondsWhereInput[], coupon?: string[]): void {
     ...
     */
    private static getMaturityDateRange(range: string): DataBaseSchema.BondsWhereInput {
        const now = new Date();
     ...
     */
    private static getCouponRateRange(range: string): DataBaseSchema.BondsWhereInput {
        const couponRanges = {
            "4-7": {
